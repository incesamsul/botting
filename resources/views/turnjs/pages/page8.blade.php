
<img src="{{ asset('turnjs/invitation/pics/bg/' . $nama_tema . '/tl.png?p') }}" style="width:100%; height:100%;position: absolute; z-index: 1">
<div class="book-content">

    <div class="mempelai-wrapper  text-center text-{{ $nama_tema }} ">
        <span class="mempelai ">Akad nikah</span>
        <i class="icon fa-regular fa-calendar-check"></i>
        <span class="text-main mt-2">Senin, 22 oktober 2022</span>
        <span class="text-main mt-2">11:00 WITA - Selesai</span>
        <i class="fa-solid fa-mountain-city icon mt-2"></i>
        <span class="text-main mt-2">Kediaman memplay wanita</span>
        <span class="text-main mt-2">Shiganshina-ku? is a town located on the south edge of Wall Maria and is the hometown of Eren Yeager, .</span>
    </div>

</div>
<span class="page-number">8</span>
